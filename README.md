...
### 4. samples/ の中身を全部まとめて一気通貫で回す

```bash
mise run run-all
# mise が無い場合: python scripts/run_all.py
```

`samples/`にある画像すべてに対して手順3と同じ`run`（前処理＋Gemini判定）を実行し、
`output/<ファイル名>.png` / `.json`を出力する。1件失敗しても残りは続行し、
最後に成功/失敗と所要時間の一覧が出る（生成時間・成功率の記録用）。
